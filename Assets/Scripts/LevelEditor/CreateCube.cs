...
/*        public void WriteFileByLine(string file_path, string file_name, string str_info) {
        StreamWriter sw;
            File.CreateText(file_path + "//" + file_name);//创建一个用于写入 UTF-8 编码的文本
            Debug.Log("文件创建成功！");
...
/*            WriteFileByLine("", "level"+"123", currLevel_s);*/
        ...
        void getRay() {
            ...
            isHit = Physics.Raycast((Ray)ray, out hit);             //发出射线检测到了碰撞   isHit返回的是 一个bool值
        ...
        void MouseStateManager() {
            ...
            if (MouseState == 1 && Input.GetMouseButtonUp(0)) {
                MouseState = 2;
                mousetimer = (float)(fixupdatetime * mouseinterval / 3);
            }
